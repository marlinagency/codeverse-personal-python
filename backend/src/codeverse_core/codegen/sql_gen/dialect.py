"""PostgreSQL dialect rules: type mapping, literals, and the runtime prelude.

Design decisions (see docs/sql-concept-mapping.md):

* Lists AND dicts are both ``jsonb`` — one uniform container type whose
  operations are covered by a small prelude of ``_cv_*`` helper functions
  emitted at the top of every generated script (a runtime library, the same
  technique real compilers use).
* All numbers are ``numeric`` so int/float unify; numeric values are cast to
  ``::numeric`` before entering jsonb so equality is stable.
* List indexing stays 0-based (jsonb arrays are 0-based natively).
"""

from __future__ import annotations

#: DSL type name -> PostgreSQL type
...
#: DSL import name -> PostgreSQL extension (the EMULATED import whitelist)
EXTENSION_WHITELIST: dict[str, str] = {
    "pgcrypto": "pgcrypto",
    "uuid_ossp": "uuid-ossp",
    "hstore": "hstore",
    "tablefunc": "tablefunc",
...
#: Helper-function prelude prepended to every generated script.
PRELUDE = """\

...
DROP TYPE IF EXISTS Oyuncu CASCADE;
CREATE TYPE Oyuncu AS (
  isim text,
  puan numeric
);

CREATE OR REPLACE FUNCTION Oyuncu_puan_ekle(self Oyuncu, eklenecek numeric)
RETURNS Oyuncu
...
BEGIN
  ...
  RAISE NOTICE '%', coalesce((o).isim, 'None');
  RAISE NOTICE '%', coalesce(((o).puan)::text, 'None');
END
$main$;
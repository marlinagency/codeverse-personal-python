...
CREATE OR REPLACE FUNCTION fib(n numeric)
RETURNS numeric
...
BEGIN
  sonuclar := '[]'::jsonb;
  FOR i IN 0..((10)::int) - 1 LOOP
    sonuclar := _cv_append(sonuclar, to_jsonb((fib(i))::numeric));
  END LOOP;
  RAISE NOTICE '%', coalesce(_cv_str(sonuclar), 'None');
END
$main$;
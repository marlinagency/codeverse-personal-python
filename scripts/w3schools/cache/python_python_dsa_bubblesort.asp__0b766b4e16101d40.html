...
<html lang="en-US">
<head>
<title>Bubble Sort with Python</title>
<meta charset="utf-8">
...
.v-enter-from, .miniSort-enter-from {
  translate: 0 100px;
}
...
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="python_dsa_binarysearch.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="python_dsa_selectionsort.asp">Next &#10095;</a>
</div>
<hr>
  ...
  methods: {
    ...
    async bblSort() {
      ...
      for (var i = 0; i < this.dice.length; i++) {
        for (var j = 0; j < (this.dice.length - i - 1); j++) {
          this.dice[j].isSwapped = true;
          this.dice[j + 1].isSwapped = true;
          await new Promise(resolve => setTimeout(resolve, this.delay));
          if (this.dice[j].dieNmbr > this.dice[j + 1].dieNmbr) {
            var temp = this.dice.splice(j, 1)[0];
            this.dice.splice(j + 1, 0, temp);
            await new Promise(resolve => setTimeout(resolve, this.delay));
          }
          this.dice[j].isSwapped = false;
          this.dice[j + 1].isSwapped = false;
        }
        this.dice[this.dice.length-i-1].isFinished = true;
      }
      this.msgDone = 'Done!';
...
<script>
  ...
  methods: {
    action(){
      if(this.buttonText === "Bubble Sort") {
        this.bblSort();
      }
      else {
        this.addDie50();
    ...
    async bblSort() {
      ...
      // Outer for-loop to handle multiple passes
      for (let i = 0; i < n - 1; i++) {
        for (let j = 0; j < n - i - 1; j++) {
          ...
          if (this.dice[j].dieNmbr > this.dice[j + 1].dieNmbr) {
            const temp = this.dice.splice(j, 1)[0];
            this.dice.splice(j + 1, 0, temp);
...
<a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_bubblesort" target="_blank">Run Example &raquo;</a>
</div>
...
<a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_bubblesort2" target="_blank">Run Example &raquo;</a>
</div>
...
<img src="/dsa/img_runtime_bubbleSort.png" alt="Bubble Sort time complexity">
<p>As you can see, the run time increases really fast when the size of the array is increased.</p>
<p>Luckily there are sorting algorithms that are faster than this, like <a href="python_dsa_quicksort.asp">Quicksort</a>, that we will look at later.</p>
<hr>

...
<html lang="en-US">
<head>
<title>Binary Search with Python</title>
<meta charset="utf-8">
...
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="python_dsa_linearsearch.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="python_dsa_bubblesort.asp">Next &#10095;</a>
</div>
<hr>
...
const app = Vue.createApp({
  ...
  methods: {
    action(){
      if(this.buttonText === "Search for value 2") {
        this.binarySearch();
      }
      else {
        this.addDie50();
    ...
    async binarySearch() {
      ...
      this.inpIsDisabled = true;
      let left = 0;
      let right = this.dice.length-1;

      while (left <= right) {
        this.currIndex = Math.floor((left + right) / 2);
        this.currVal = this.dice[this.currIndex].dieNmbr;
        this.targetPos = this.currIndex+1;
        await new Promise(resolve => setTimeout(resolve, 150));
        this.dice[this.currIndex].isActive = true;
        this.dice[this.currIndex].isBorder = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));

        if (this.dice[this.currIndex].dieNmbr === this.targetHeight) {
          this.currVal = this.dice[this.currIndex].dieNmbr;
          this.targetIndex = this.currIndex;
          for(let i=0; i<this.dice.length; i++){
            if(i !== this.currIndex){
              this.dice[i].isDone = true;
            }
          }
          await new Promise(resolve => setTimeout(resolve, this.delay));
          break;
        ...
        if (this.dice[this.currIndex].dieNmbr < this.targetHeight) {
          ...
          right = this.currIndex - 1;
          for(let i=this.currIndex; i<this.dice.length; i++){
            this.dice[i].isDone = true;
          }
          await new Promise(resolve => setTimeout(resolve, this.delay));
      ...
      if(this.dice[this.currIndex].dieNmbr !== this.targetHeight){
        this.msgDone = 'Not found! Return: -1';
        ...
        this.msgDone = 'Found it! Index: '+this.currIndex;
        this.buttonText = "Reset";
        this.btnIsDisabled = false;
        for(let i=this.currIndex+1; i<this.dice.length; i++){
          this.dice[i].isDone = true;
        }
        this.dice[this.targetIndex].isFinished = true;
        this.dice[this.targetIndex].isBorder = true;
      }
    },
    newRandom(){
      let rand = 5;
    ...
    addDie50() {
      ...
      this.targetPos = 0;
      this.inpIsDisabled = false;
      for (let i = 0; i < 21; i++) {
        this.addDie();
      }
      this.buttonText = "Search for value 2";
      this.msgDone = '';
      this.shuffleSort();
    },
    compareFunc(a, b) {
    ...
    shuffleSort(){
      this.dice.sort(this.compareFunc);
    }
  },
  mounted() {
    this.addDie50();
...
const app2 = Vue.createApp({
  data() {
    return {
      ...
      dice: [],
      keyNumber: 0,
      buttonText: 'Search for value 11'
    }
  },
  computed: {
    formattedDiceArray() {
  ...
  methods: {
    action(){
      if(this.buttonText === "Search for value 11") {
        this.binarySearch();
      }
      else {
        this.addDie50();
    ...
    updateMoveDuration() {
      ...
      for (let i = 0; i < stylesheets.length; i++) {
        let rules = stylesheets[i].cssRules || stylesheets[i].rules;
        for (let j = 0; j < rules.length; j++) {
          if (rules[j].selectorText === '.v2-move') {
            rules[j].style.transitionDuration = this.delay + 'ms';
            break;
          }
        }
    ...
    async binarySearch() {
      ...
      let right = this.dice.length-1;
      while(right-left >= 0){
        this.currentIndex = Math.floor((right+left)/2);
        await new Promise(resolve => setTimeout(resolve, 150));
        this.dice[this.currentIndex].isActive = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        if(this.dice[this.currentIndex].dieNmbr === 11){
          await new Promise(resolve => setTimeout(resolve, this.delay));
          break;
        }
        this.dice[this.currentIndex].isActive = false;
        if(this.dice[this.currentIndex].dieNmbr < 11){
          left = this.currentIndex + 1;
        }
        else {
          right = this.currentIndex - 1;
        }
        
      }
      this.msgDone = 'Found value 11 at index 4!';
      this.buttonText = "Reset";
      this.btnIsDisabled = false;
    },
    addDie50() {
      ...
      this.buttonText = "Search for value 11";
      this.msgDone = '';
    }
  },
  mounted() {
...
&nbsp;&nbsp;print("Not found")</div>
  <a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_binarysearch" target="_blank">Run Example &raquo;</a>
</div>
<hr>
...
<img src="/dsa/img_binarysearch_timecomplexity.png" class="imgTut" alt="Binary Search Time Complexity">
<hr>

...
<html lang="en-US">
<head>
<title>SQL COALESCE(), IFNULL(), ISNULL(), NVL() Functions</title>
<meta charset="utf-8">
...
<h2>SQL COALESCE(), IFNULL(), ISNULL(), and NVL() Functions</h2>
<p>Operations involving NULL values can sometimes lead to unexpected results.</p>
<p>SQL has some built-in functions to handle NULL values, and the most common functions are:</p>
<ul>
  <li><code class="w3-codespan">COALESCE()</code> - The preferred standard. (Works in MySQL, SQL Server and Oracle)</li>
  <li><code class="w3-codespan">IFNULL()</code> - (MySQL)</li>
  <li><code class="w3-codespan">ISNULL()</code> - (SQL Server)</li>
  <li><code class="w3-codespan">NVL()</code> - (Oracle)</li>
  <li><code class="w3-codespan">IsNull()</code> - (MS Access)</li>
</ul>

<div class="w3-panel ws-note">
<p><b>Note:</b> A <code class="w3-codespan">NULL</code> value represents an unknown or missing data in a database field. It is not a value itself, but a placeholder to indicate the absence of data.</p>
</div>
<hr>

<h2>Demo Database</h2>
<p>Assume we have the following &quot;Products&quot; table:</p>
...
<p>The &quot;InOrder&quot; column is optional, and may contain NULL values.</p>
<p>Now look at the following SQL statement:</p>

<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
SELECT ProductName, Price * (InStock + InOrder)<br>
FROM Products;</div>
</div>
<p><strong>Note: In the SQL above, if any of the &quot;InOrder&quot; values are NULL, the result will be NULL!</strong></p>
<hr>
...
<p>The <code class="w3-codespan">COALESCE()</code> function is the preferred standard for handling potential NULL values.</p>
<p>The <code class="w3-codespan">COALESCE()</code> function returns the first non-NULL value in a list 
of values.</p>
<p>The <code class="w3-codespan">COALESCE()</code> function works in <a href="func_mysql_coalesce.asp">MySQL</a>,
<a href="func_sqlserver_coalesce.asp">SQL Server</a>, and Oracle (not in MS Access).</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
COALESCE(<em>val1</em>, <em>val2</em>, <em>....</em>, <em>val_n</em>)</div>
<p>Here we use the <code class="w3-codespan">
COALESCE()</code> function to replace NULL 
values with 0:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
SELECT ProductName, Price * (InStock + COALESCE(InOrder, 0))<br>
FROM Products;</div></div>
<hr>

<h2>The IFNULL() Function (MySQL)</h2>
<p>The MySQL <code class="w3-codespan"><a href="func_mysql_ifnull.asp">IFNULL()</a></code> 
function replaces NULL with a specified value.</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
  IFNULL(<em>expr</em>, <em>alt</em>)</div>

<p>Here we replace NULL values with 0:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
SELECT ProductName, Price * (InStock + IFNULL(InOrder, 0))<br>
FROM Products;</div></div>
<hr>

<h2>The ISNULL() Function (SQL Server)</h2>
<p>The SQL Server <code class="w3-codespan"><a href="func_sqlserver_isnull.asp">
ISNULL()</a></code> function replaces NULL with a specified value.</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
  ISNULL(<em>expr</em>, <em>alt</em>)</div>

<p>Here we replace NULL values with 0:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
SELECT ProductName, Price * (InStock + ISNULL(InOrder, 0))<br>
FROM Products;</div></div>
<hr>

<h2>The NVL() Function (Oracle)</h2>
<p>The Oracle <code class="w3-codespan">NVL()</code> function replaces NULL with 
a specified value.</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
NVL(<em>expr</em>, <em>alt</em>)</div>
<p>Here we replace NULL values with 0:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
SELECT ProductName, Price * (InStock + NVL(InOrder, 0))<br>
FROM Products;</div></div>
<hr>

<h2>The IsNull() Function (MS Access)</h2>
<p>The MS Access <code class="w3-codespan"><a href="func_msaccess_isnull.asp">
IsNull()</a></code> function returns TRUE if the expression is NULL, otherwise FALSE.</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
  IsNull(<em>expr</em>)</div>

<p>The MS Access <code class="w3-codespan">IIf()</code> function returns one 
of two parts, depending on the evaluation of the expression.</p>

<h3>Syntax</h3>
<div class="w3-code w3-border notranslate">
  IIf(<em>expr, truepart, falsepart</em>)</div>

<ul>
  <li><em>expr</em> - Required. The expression to evaluate</li>
  <li><em>truepart</em> - Value to return if <em>expr</em> is True</li>
  <li><em>falsepart</em> - Value to return if <em>expr</em> is False</li>
</ul>
<p>Here we replace NULL values with 0:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
...
FROM Products;</div></div>
<hr>
<div id="exercisecontainer" src="xrcise_isnull.js"></div>
<hr>
<br>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="sql_case.asp">&#10094; Previous</a>

...
<html lang="en-US">
<head>
<title>Python Machine Learning - AUC - ROC Curve</title>
<meta charset="utf-8">
...
<h1>Machine Learning - AUC - ROC Curve</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_cross_validation.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_knn.asp">Next &#10095;</a>
</div>
<hr>

<h2>AUC - ROC Curve</h2>

<p>In classification, there are many different evaluation metrics. The most popular is <strong>accuracy</strong>, which measures how often the model is correct. This is a great metric because it is easy to understand and getting the most correct guesses is often desired. There are some cases where you might consider using another evaluation metric.</p>

<p>Another common metric is <strong>AUC</strong>, area under the receiver operating characteristic (<strong>ROC</strong>) curve.
The Reciever operating characteristic curve plots the true positive (<strong>TP</strong>) rate versus the false positive (<strong>FP</strong>) rate at different classification thresholds. The thresholds are different probability cutoffs that separate the two classes in binary classification. It uses probability to tell us how well a model separates the classes.</p>

<hr>

<h2>Imbalanced Data</h2>

<p>Suppose we have an imbalanced data set where the majority of our data is of one value. We can obtain high accuracy for the model by predicting the majority class.</p>

<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
import numpy as np<br>
from sklearn.metrics import accuracy_score, confusion_matrix, roc_auc_score, roc_curve<br>
<br>
n = 10000<br>
ratio = .95<br>
n_0 = int((1-ratio) * n)<br>
n_1 = int(ratio * n)<br>
<br>
y = np.array([0] * n_0 + [1] * n_1)<br>
# below are the probabilities obtained from a hypothetical model that always predicts the majority class<br>
# probability of predicting class 1 is going to be 100%<br>
y_proba = np.array([1]*n)<br>
y_pred = y_proba > .5<br>
<br>
print(f'accuracy score: {accuracy_score(y, y_pred)}')<br>
cf_mat = confusion_matrix(y, y_pred)<br>
print('Confusion matrix')<br>
print(cf_mat)<br>
print(f'class 0 accuracy: {cf_mat[0][0]/n_0}')<br>
print(f'class 1 accuracy: {cf_mat[1][1]/n_1}')
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc1">Run example &raquo;</a>
</div>
<hr>

<p>Although we obtain a very high accuracy, the model provided no information about the data so it's not useful. We accurately predict class 1 100% of the time while inaccurately predict class 0 0% of the time. At the expense of accuracy, it might be better to have a model that can somewhat separate the two classes.</p>

<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
# below are the probabilities obtained from a hypothetical model that doesn't always predict the mode<br>
y_proba_2 = np.array(<br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(0, .7, n_0).tolist() + <br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.3, 1,  n_1).tolist()<br>
)<br>
y_pred_2 = y_proba_2 > .5<br>
<br>
print(f'accuracy score: {accuracy_score(y, y_pred_2)}')<br>
cf_mat = confusion_matrix(y, y_pred_2)<br>
print('Confusion matrix')<br>
print(cf_mat)<br>
print(f'class 0 accuracy: {cf_mat[0][0]/n_0}')<br>
print(f'class 1 accuracy: {cf_mat[1][1]/n_1}')</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc2">Run example &raquo;</a>
</div>

<p>For the second set of predictions, we do not have as high of an accuracy score as the first but the accuracy for each class is more balanced. Using accuracy as an evaluation metric we would rate the first model higher than the second even though it doesn't tell us anything about the data.</p>

<p>In cases like this, using another evaluation metric like AUC would be preferred.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">import matplotlib.pyplot as plt<br>
<br>
def plot_roc_curve(true_y, y_prob):<br>
&nbsp;&nbsp;&nbsp;&nbsp;"""<br>
&nbsp;&nbsp;&nbsp;&nbsp;plots the roc curve based of the probabilities<br>
&nbsp;&nbsp;&nbsp;&nbsp;"""<br>
<br>    
&nbsp;&nbsp;&nbsp;&nbsp;fpr, tpr, thresholds = roc_curve(true_y, y_prob)<br>
&nbsp;&nbsp;&nbsp;&nbsp;plt.plot(fpr, tpr)<br>
&nbsp;&nbsp;&nbsp;&nbsp;plt.xlabel('False Positive Rate')<br>
&nbsp;&nbsp;&nbsp;&nbsp;plt.ylabel('True Positive Rate')</code></p>
</div>
...
<p>Model 1:</p>
<div class="w3-code notranslate pythonHigh">
plot_roc_curve(y, y_proba)<br>
print(f'model 1 AUC score: {roc_auc_score(y, y_proba)}')    
    </div>
<h3>Result</h3>
<p>
<img src="img_ml_auc_roc3.png" style="max-width:100%">
<br>model 1 AUC score: 0.5
<br>
</p>
    
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc3">Run example &raquo;</a>
</div>
...
<p>Model 2:</p>
<div class="w3-code notranslate pythonHigh">
plot_roc_curve(y, y_proba_2)<br>
print(f'model 2 AUC score: {roc_auc_score(y, y_proba_2)}') 
    </div>
<h3>Result</h3>
<p>
<img src="img_ml_auc_roc4.png" style="max-width:100%">
<br>model 2 AUC score: 0.8270551578947367
</p>
    
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc4">Run example &raquo;</a>
</div>

<p>An AUC score of around .5 would mean that the model is unable to make a distinction between the two classes and the curve would look like a line with a slope of 1. An AUC score closer to 1 means that the model has the ability to separate the two classes and the curve would come closer to the top left corner of the graph.</p>
...
<p>Because AUC is a metric that utilizes probabilities of the class predictions, we can be more confident in a model that has a higher AUC score than one with a lower score even if they have similar accuracies.</p>

<p>In the data below, we have two sets of probabilites from hypothetical models. The first has probabilities that are not as "confident" when predicting the two classes (the probabilities are close to .5). The second has probabilities that are more "confident" when predicting the two classes (the probabilities are close to the extremes of 0 or 1).</p>
...
<br>
n = 10000<br>
y = np.array([0] * n + [1] * n)<br>
# <br>
y_prob_1 = np.array(<br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.25, .5, n//2).tolist() + <br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.3, .7, n).tolist() + <br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.5, .75, n//2).tolist()<br>
)<br>
y_prob_2 = np.array(<br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(0, .4, n//2).tolist() + <br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.3, .7, n).tolist() + <br>
&nbsp;&nbsp;&nbsp;&nbsp;np.random.uniform(.6, 1, n//2).tolist()<br>
)<br>
<br>
print(f'model 1 accuracy score: {accuracy_score(y, y_prob_1>.5)}')<br>
print(f'model 2 accuracy score: {accuracy_score(y, y_prob_2>.5)}')<br>
<br>
print(f'model 1 AUC score: {roc_auc_score(y, y_prob_1)}')<br>
print(f'model 2 AUC score: {roc_auc_score(y, y_prob_2)}')</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc5">Run example &raquo;</a>
</div>
...
<p>Plot model 1:</p>
<div class="w3-code notranslate pythonHigh">
plot_roc_curve(y, y_prob_1)</div>
<h3>Result</h3>
<p><img src="img_ml_auc_roc6.png" style="max-width:100%"></p>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc6">Run example &raquo;</a>
</div>
...
<p>Plot model 2:</p>
<div class="w3-code notranslate pythonHigh">
fpr, tpr, thresholds = roc_curve(y, y_prob_2)<br>
plt.plot(fpr, tpr)</div>
<h3>Result</h3>
<p><img src="img_ml_auc_roc7.png" style="max-width:100%"></p>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_auc_roc7">Run example &raquo;</a>
</div>

<p>Even though the accuracies for the two models are similar, the model with the higher AUC score will be more reliable because it takes into account the predicted probability. It is more likely to give you higher accuracy when predicting future data.</p>
<hr>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_cross_validation.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_knn.asp">Next &#10095;</a>

...
<html lang="en-US">
<head>
<title>Python Machine Learning - Cross Validation</title>
<meta charset="utf-8">
...
<h1>Machine Learning - Cross Validation</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_bagging.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_auc_roc.asp">Next &#10095;</a>
</div>
<hr>

<h2>Cross Validation</h2>

<p>When adjusting models we are aiming to increase overall model performance on unseen data. Hyperparameter tuning can lead to much better performance on test sets. However, optimizing parameters to the test set can lead information leakage causing the model to preform worse on unseen data. To correct for this we can perform cross validation.</p>

<p>To better understand CV, we will be performing different methods on the iris dataset. Let us first load in and separate the data.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">from sklearn import datasets<br>
<br>
X, y = datasets.load_iris(return_X_y=True)</code></p>
</div>

<p>There are many methods to cross validation, we will start by looking at k-fold cross validation.</p>
<hr>

<h2><em>K</em>-Fold</h2>

<p>The training data used in the model is split, into k number of smaller sets, to be used to validate the model. The model is then trained on k-1 folds of training set. The remaining fold is then used as a validation set to evaluate the model.</p>

<p>As we will be trying to classify different species of iris flowers we will need to import a classifier model, for this exercise we will be using a <code class="w3-codespan">DecisionTreeClassifier</code>. We will also need to import CV modules from <code class="w3-codespan">sklearn</code>.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">from sklearn.tree import DecisionTreeClassifier<br>
from sklearn.model_selection import KFold, cross_val_score</code></p>
</div>

<p>With the data loaded we can now create and fit a model for evaluation.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">clf = DecisionTreeClassifier(random_state=42)</code></p>
</div>

<p>Now let's evaluate our model and see how it performs on each <em>k</em>-fold.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">k_folds = KFold(n_splits = 5)<br>
<br>
scores = cross_val_score(clf, X, y, cv = k_folds)</code></p>
</div>

<p>It is also good practice to see how CV performed overall by averaging the scores for all folds.</p>
...
<br>
clf = DecisionTreeClassifier(random_state=42)<br>
<br>
k_folds = KFold(n_splits = 5)<br>
<br>
scores = cross_val_score(clf, X, y, cv = k_folds)<br>
<br>
print("Cross Validation Scores: ", scores)<br>
print("Average CV Score: ", scores.mean())<br>
print("Number of CV Scores used in Average: ", len(scores))</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_k-fold">Run example &raquo;</a>
</div>
<hr>
...
<p>In cases where classes are imbalanced we need a way to account for the imbalance in both the train and validation sets. To do so we can stratify the target classes, meaning that both sets will have an equal proportion of all classes.</p>
...
clf = DecisionTreeClassifier(random_state=42)<br>
<br>
sk_folds = StratifiedKFold(n_splits = 5)<br>
<br>
scores = cross_val_score(clf, X, y, cv = sk_folds)<br>
<br>
print("Cross Validation Scores: ", scores)<br>
print("Average CV Score: ", scores.mean())<br>
print("Number of CV Scores used in Average: ", len(scores))
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_stratified_k-fold">Run example &raquo;</a>
</div>

<p>While the number of folds is the same, the average CV increases from the basic k-fold when making sure there is stratified classes.</p>

<hr>

<h2>Leave-One-Out (LOO)</h2>

<p>Instead of selecting the number of splits in the training data set like k-fold LeaveOneOut, utilize 1 observation to validate and n-1 observations to train. This method is an exaustive technique.</p>
...
from sklearn import datasets<br>
from sklearn.tree import DecisionTreeClassifier<br>
from sklearn.model_selection import LeaveOneOut, cross_val_score<br>
<br>
X, y = datasets.load_iris(return_X_y=True)<br>
<br>
clf = DecisionTreeClassifier(random_state=42)<br>
...
loo = LeaveOneOut()<br>
<br>
scores = cross_val_score(clf, X, y, cv = loo)<br>
<br>
print("Cross Validation Scores: ", scores)<br>
print("Average CV Score: ", scores.mean())<br>
print("Number of CV Scores used in Average: ", len(scores))
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_loo">Run example &raquo;</a>
</div>

<p>We can observe that the number of cross validation scores performed is equal to the number of observations in the dataset. In this case there are 150 observations in the iris dataset.</p>

<p>The average CV score is 94%.</p>

<hr>

<h2>Leave-P-Out (LPO)</h2>

<p>Leave-P-Out is simply a nuanced diffence to the Leave-One-Out idea, in that we can select the number of p to use in our validation set.</p>
...
from sklearn import datasets<br>
from sklearn.tree import DecisionTreeClassifier<br>
from sklearn.model_selection import LeavePOut, cross_val_score<br>
<br>
X, y = datasets.load_iris(return_X_y=True)<br>
<br>
clf = DecisionTreeClassifier(random_state=42)<br>
<br>
lpo = LeavePOut(p=2)<br>
<br>
scores = cross_val_score(clf, X, y, cv = lpo)<br>
<br>
print("Cross Validation Scores: ", scores)<br>
print("Average CV Score: ", scores.mean())<br>
print("Number of CV Scores used in Average: ", len(scores))
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_lpo">Run example &raquo;</a>
</div>

<p>As we can see this is an exhaustive method we many more scores being calculated than Leave-One-Out, even with a p = 2, yet it achieves roughly the same average CV score.</p>

<hr>

<h2>Shuffle Split</h2>

<p>Unlike <code class="w3-codespan">KFold</code>, <code class="w3-codespan">ShuffleSplit</code> leaves out a percentage of the data, not to be used in the train or validation sets. To do so we must decide what the train and test sizes are, as well as the number of splits.</p>


<div class="w3-example">
<h3>Example</h3>
<p>Run Shuffle Split CV:</p>
<div class="w3-code notranslate pythonHigh">
from sklearn import datasets<br>
from sklearn.tree import DecisionTreeClassifier<br>
from sklearn.model_selection import ShuffleSplit, cross_val_score<br>
<br>
X, y = datasets.load_iris(return_X_y=True)<br>
<br>
clf = DecisionTreeClassifier(random_state=42)<br>
<br>
ss = ShuffleSplit(train_size=0.6, test_size=0.3, n_splits = 5)<br>
<br>
scores = cross_val_score(clf, X, y, cv = ss)<br>
<br>
print("Cross Validation Scores: ", scores)<br>
print("Average CV Score: ", scores.mean())<br>
print("Number of CV Scores used in Average: ", len(scores))
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_shuffle_split">Run example &raquo;</a>
</div>
<hr>

<h2>Ending Notes</h2>

<p>These are just a few of the CV methods that can be applied to models. There are many more cross validation classes, with most models having their own class. Check out sklearns cross validation for more CV options.</p>
<hr>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_bagging.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_auc_roc.asp">Next &#10095;</a>

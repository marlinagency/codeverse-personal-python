...
<html lang="en-US">
<head>
<title>SQL Prepared Statements</title>
<meta charset="utf-8">
...
<h1>SQL <span class="color_h1">Prepared Statements</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="sql_parameterized_queries.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="sql_hosting.asp">Next &#10095;</a>
</div>
<hr>

<h2>SQL Prepared Statements - Prevent SQL Injection</h2>
<p>SQL prepared statements can be used to protect a web site from SQL injections.</p>
<p>Prepared statements seperates the query structure (the SQL) from the actual 
data (user input).</p>
<p>Prepared statements basically work like this:</p>
<ol>
  <li><strong>Prepare:</strong> An SQL query template with placeholders is sent to the 
  server. The data values are not sent. Example: INSERT INTO MyGuests VALUES(?, ?, ?). 
  Then, the server parses, compiles, and optimizes the SQL query template, without executing it</li>
  <li><strong>Execute:</strong> At a later time, the application binds the values to the parameters, and the database executes the 
  query. The application may execute the query as many times as it wants with different values</li>
</ol>
<p>Prepared statements have four main advantages:</p>
<ul>
  <li><strong>Reduced parsing time</strong> - as the preparation on the query is done only once (although the statement is executed multiple times)</li>
  <li><strong>Minimize bandwidth</strong> - Bound parameters minimize bandwidth to the server as you need send only the parameters each time, and not the whole query</li>
  <li><strong>Security</strong> - Prepared statements are very useful against 
  ...
  template is not derived from external input, SQL injection cannot occur</li>
  <li><strong>Cleaner code</strong> - by seperating data from SQL commands</li>
</ul>
<hr>

<h2>Prepared Statements in MySQL</h2>
<p>The following example is taken from <a href="/php/php_mysql_prepared_statements.asp">PHP MySQL Prepared Statements</a>, 
and uses prepared statements in MySQL:</p>
<div class="w3-example">
<h3>Example - MySQL with Prepared Statements</h3>
...
&lt;?php<br>$servername = &quot;localhost&quot;;<br>$username = &quot;username&quot;;<br>$password = &quot;password&quot;;<br>$dbname = &quot;myDB&quot;;<br><br>// Create connection<br>$conn = new mysqli($servername, $username, $password, $dbname);<br><br>// Check connection<br>if ($conn-&gt;connect_error) {<br>&nbsp;&nbsp;die(&quot;Connection failed: &quot; . $conn-&gt;connect_error);<br>
 }<br><br>// SQL query template<br>$sql = &quot;INSERT INTO MyGuests (firstname, lastname, email) VALUES (?, ?, ?)&quot;;<br>
  <br>// Prepare the SQL query template<br>if($stmt = $conn-&gt;prepare($sql)) {<br>
  &nbsp; // Bind parameters<br>&nbsp; $stmt-&gt;bind_param(&quot;sss&quot;, $firstname, $lastname, $email);<br><br>&nbsp; // 
  Set parameters and execute<br>&nbsp; $firstname = &quot;John&quot;;<br>&nbsp; $lastname = &quot;Doe&quot;;<br>&nbsp; $email = &quot;john@example.com&quot;;<br>&nbsp; $stmt-&gt;execute();<br><br>&nbsp; $firstname = &quot;Mary&quot;;<br>&nbsp; $lastname = &quot;Moe&quot;;<br>&nbsp; $email = &quot;mary@example.com&quot;;<br>&nbsp; $stmt-&gt;execute();<br>
 <br>&nbsp; $firstname = &quot;Julie&quot;;<br>&nbsp; $lastname = &quot;Dooley&quot;;<br>&nbsp; $email = &quot;julie@example.com&quot;;<br>
 &nbsp;
 $stmt-&gt;execute();<br>&nbsp; echo &quot;New records created successfully&quot;;<br>
  } else {<br>&nbsp; echo &quot;Error: &quot; . $sql . &quot;&lt;br&gt;&quot; . $conn-&gt;error;<br>}<br><br>$stmt-&gt;close();<br>
 $conn-&gt;close();<br>?&gt;
</div>
<br>
</div>
<h3>Code Explanation</h3>
<p>In the SQL, the question marks (?) are placeholders for <em>firstname</em>,
<em>lastname</em>, and <em>email</em> values:&nbsp;</p>
<div class="w3-example">
<div class="w3-code notranslate">
 &quot;INSERT INTO MyGuests (firstname, lastname, email) VALUES (?, ?, ?)&quot;</div>
</div>
<p>Now, look at the <code class="w3-codespan">
bind_param()</code> function. This function bind variables to the placeholders 
...
<strong>type</strong> of data each parameter is. The <strong>s</strong> 
...
<p>The type argument can be one of four types:</p>
<ul>
  ...
  <li><strong>b</strong> - binary (image, PDF, etc.)</li>
</ul>

<div class="w3-panel ws-note">
<p><strong>Note:</strong> If we want to insert data from external sources 

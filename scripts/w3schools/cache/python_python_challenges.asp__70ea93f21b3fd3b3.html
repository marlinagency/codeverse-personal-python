...
<html lang="en-US">
<head>
<title>Python Code Challenges</title>
<meta charset="utf-8">
...
.challenge_tiles a { text-decoration:none; color:inherit; }
.challenge_toolbar{
  display:flex;
  align-items:center;
  gap:10px;
...
#challenge_filter:focus{
  outline:none;
  border-color:#059862;
  box-shadow:0 0 0 3px rgba(4,170,109,0.2);
}
.challenge_tile .tile-content {
  display:flex;
  align-items:center;
  flex:1;
}
.challenge_tile .open-btn {
  background:#ffffff;
  border:1px solid #D1D5DB;
  border-radius:5px;
  padding:6px 14px;
...
<h1>Python <span class="color_h1">Code Challenges</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_quiz.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_practice.php">Next &#10095;</a>
</div>
<hr>
<p>Test your Python skills with code challenges from all categories:</p>

<div class="challenge_tiles">
<h2 class="heading">Code Challenges</h2>
<div class="challenge_toolbar">
  <input id="challenge_filter" type="search" placeholder="Filter challenges (e.g. loops, functions, classes)" aria-label="Filter challenges">
</div>
<div id="challenges_container">
  <div id="challenges_loading">Loading challenges...</div>
</div>
</div>

<script>
(function() {
  var menu = document.getElementById('leftmenuinnerinner');
  if (!menu) {
    document.getElementById('challenges_container').innerHTML = '<p>Error: Menu not found.</p>';
    return;
  }
  
  var challenges = [];
  var overviews = menu.querySelectorAll('.tut_overview');
  overviews.forEach(function(overview) {
    var challengeLink = overview.querySelector('a[href*="python_challenges_"]');
    if (challengeLink) {
      var href = challengeLink.getAttribute('href');
      var headerLink = overview.previousElementSibling;
      while (headerLink && headerLink.tagName !== 'A') {
        headerLink = headerLink.previousElementSibling;
      }
      var title = headerLink ? headerLink.textContent.trim() : href.replace('python_challenges_', '').replace('.asp', '');
      title = title.replace(/^Python\s*/i, '');
      challenges.push({ title: title, href: href });
    }
  });
  
  var container = document.getElementById('challenges_container');
  ...
  challenges.forEach(function(ch) {
    tilesHtml += '<a href="' + ch.href + '">';
    tilesHtml += '<div class="challenge_tile">';
    tilesHtml += '<div class="tile-content">';
    tilesHtml += '<h2>' + ch.title + '</h2>';
    tilesHtml += '</div>';
    tilesHtml += '<button class="open-btn" aria-label="Open ' + ch.title + ' challenge">Open</button>';
    tilesHtml += '</div>';
    tilesHtml += '</a>';
  });
  container.innerHTML = tilesHtml;
  
  var filterInput = document.getElementById('challenge_filter');
  if (filterInput) {
    filterInput.addEventListener('input', function() {
      var searchTerm = this.value.toLowerCase().trim();
      var tiles = document.querySelectorAll('.challenge_tile');
...
<p>Code challenges are interactive coding exercises where you write Python code to achieve a specific result.</p>
<p>Each challenge presents you with a goal and lets you practice writing real Python code.</p>

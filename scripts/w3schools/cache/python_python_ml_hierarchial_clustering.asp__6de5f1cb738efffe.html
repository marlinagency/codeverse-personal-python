...
<html lang="en-US">
<head>
<title>Python Machine Learning - Hierarchical Clustering</title>
<meta charset="utf-8">
...
<h1>Machine Learning - Hierarchical Clustering</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_confusion_matrix.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_logistic_regression.asp">Next &#10095;</a>
</div>
<hr>

<h2>Hierarchical Clustering</h2>

<p>Hierarchical clustering is an unsupervised learning method for clustering data points. The algorithm builds clusters by measuring the dissimilarities between data. Unsupervised learning means that a model does not have to be trained, and we do not need a "target" variable. This method can be used on any data to visualize and interpret the relationship between individual data points.</p>

<p>Here we will use hierarchical clustering to group data points and visualize the clusters using both a dendrogram and scatter plot.</p>
...
<p>We will use Agglomerative Clustering, a type of hierarchical clustering that follows a bottom up approach. We begin by treating each data point as its own cluster. Then, we join clusters together that have the shortest distance between them to create larger clusters. This step is repeated until one large cluster is formed containing all of the data points.</p>

<p>Hierarchical clustering requires us to decide on both a distance and linkage method. We will use euclidean distance and the Ward linkage method, which attempts to minimize the variance between clusters.</p>

<div class="w3-example">
<h3>Example</h3>
<p>Start by visualizing some data points:</p>
<div class="w3-code notranslate pythonHigh">
  import numpy as np<br>import matplotlib.pyplot as plt<br><br>x = [4, 5, 10, 4, 
  3, 11, 14 , 6, 10, 12]<br>y = [21, 19, 24, 17, 16, 25, 24, 22, 21, 21]<br><br>
  plt.scatter(x, y)<br>plt.show()</div>
<h3>Result</h3>
<p><img src="img_ml_hierarchial_clustering.png" style="max-width:100%"></p>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_hierarchial_clustering">Run example &raquo;</a>
</div>
<hr>

<p>Now we compute the ward linkage using euclidean distance, and visualize it using a dendrogram:</p>

<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  import numpy as np<br>import matplotlib.pyplot as plt<br>from 
  scipy.cluster.hierarchy import dendrogram, linkage<br><br>x = [4, 5, 10, 4, 3, 
  11, 14 , 6, 10, 12]<br>y = [21, 19, 24, 17, 16, 25, 24, 22, 21, 21]<br><br>
  data = list(zip(x, y))<br><br>linkage_data = linkage(data, method='ward', 
  metric='euclidean')<br>dendrogram(linkage_data)<br><br>plt.show()</div>
<h3>Result</h3>
<p><img src="img_ml_hierarchial_clustering2.png" style="max-width:100%"></p>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_hierarchial_clustering2">Run example &raquo;</a>
</div>
...
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  import numpy as np<br>import matplotlib.pyplot as plt<br>from sklearn.cluster 
  import AgglomerativeClustering<br><br>x = [4, 5, 10, 4, 3, 11, 14 , 6, 10, 12]<br>
  y = [21, 19, 24, 17, 16, 25, 24, 22, 21, 21]<br><br>data = list(zip(x, y))<br>
  <br>hierarchical_cluster = AgglomerativeClustering(n_clusters=2,  
  linkage='ward')<br>labels = hierarchical_cluster.fit_predict(data)<br><br>
  plt.scatter(x, y, c=labels)<br>plt.show()</div>
<h3>Result</h3>
<p><img src="img_ml_hierarchial_clustering3.png" style="max-width:100%"></p>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_hierarchial_clustering3">Run example &raquo;</a>
</div>
...
<p>You can learn about the SciPy module in our <a href="/python/scipy/index.php">SciPy Tutorial</a>.</p>

<p>NumPy is a library for working with arrays and matricies in Python,
you can learn about the NumPy module in our <a href="/python/numpy/default.asp">NumPy Tutorial</a>.</p>

<p>scikit-learn is a popular library for machine learning.</p>

<p>Create arrays that resemble two variables in a dataset. Note that while we only 
...
<p>Turn the data into a set of points:</p>
...
<p>Compute the linkage between all of the different points. Here we use a simple euclidean distance measure and Ward's linkage, which seeks to minimize the variance between clusters.</p>
...
<p>Finally, plot the results in a dendrogram. This plot will show us the hierarchy of clusters from the bottom (individual points) to the top (a single cluster consisting of all data points).</p>
<p><code class="w3-codespan">plt.show()</code> lets us visualize the dendrogram instead of just the raw linkage data.</p>
...
<p>The scikit-learn library allows us to use hierarchichal clustering in a different manner. First, we initialize the 
<code class="w3-codespan">AgglomerativeClustering</code> class with 2 clusters 
and the Ward linkage.</p>
<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">hierarchical_cluster = AgglomerativeClustering(n_clusters=2, linkage='ward')
</code></p>
</div>

<p>The <code class="w3-codespan">.fit_predict</code> method can be called on our data to compute the clusters using the defined parameters across our chosen number of clusters.</p>
...
<p>Finally, if we plot the same data and color the points using the labels assigned to each index by the hierarchical clustering method, we can see the cluster each point was assigned to:</p>

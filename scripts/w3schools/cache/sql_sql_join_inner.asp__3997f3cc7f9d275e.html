...
<h1>SQL <span class="color_h1">INNER JOIN</span></h1>
...
<p>The <code class="w3-codespan">INNER JOIN</code> returns only rows that have <b>matching values</b> in both tables.</p>
<p><b>Tip:</b> You can use just <code class="w3-codespan">JOIN</code> instead of <code class="w3-codespan">INNER JOIN</code>, as <code class="w3-codespan">INNER</code> is the default join type.</p>
<p style="text-align:center;"><img class="invert-on-dark" alt="SQL INNER JOIN" src="img_inner_join.png"></p>
...
<p><strong>Note:</strong> The syntax combines two tables based on a <strong>related column</strong>, 
and the <code class="w3-codespan">ON</code> keyword is used to specify the matching condition.</p>
<hr>

<h2>INNER JOIN Example</h2>
<p>Look at a product in the <a href="trysql.asp?filename=trysql_products" target="_blank"><strong>Products</strong></a> table:</p>
...
<th>ProductID</th>
<th>ProductName</th>
<th class="auto-style1">CategoryID</th>
<th>Price</th>
</tr>
<tr>
<td>3</td>
...
<p>And look at a row in the <a href="trysql.asp?filename=trysql_categories" target="_blank"><strong>Categories</strong></a> table:</p>
...
<p>Here we see that the <strong>related column</strong> between the two tables above, is the "CategoryID" column.</p>        

<p>Now we create an <code class="w3-codespan">INNER JOIN</code> on the "Products" table and the "Categories" table, via the CategoryID field:</p>
<div class="w3-example">
<h3>Example</h3>
<p>Join "Products" and "Categories" with the INNER JOIN keyword:</p>
<div class="w3-code notranslate sqlHigh">
  SELECT ProductID, ProductName, CategoryName<br>FROM Products<br>INNER JOIN 
  Categories ON Products.CategoryID = Categories.CategoryID;</div>
<a target="_blank" href="trysql.asp?filename=trysql_select_join_inner_prod" class="w3-btn w3-margin-bottom">
 Try it Yourself &raquo;</a>
</div>

<div class="w3-panel ws-note">
<p><b>Note:</b> <code class="w3-codespan">INNER JOIN</code> returns only rows with a match in both tables.
This means that if there is a product with no CategoryID, or with a CategoryID not present in the Categories table, that row will not be returned in the result.</p>
</div>
<hr>
...
<hr>
<h2>Naming the Columns</h2>
<p>It is a good practice to also include the table name when specifying columns in SQL 
joins:</p>
<div class="w3-example">
<h3>Example</h3>
<p>Add table name in front of column names:</p>
<div class="w3-code notranslate sqlHigh">
SELECT Products.ProductID, Products.ProductName, Categories.CategoryName<br>
FROM Products<br>
INNER JOIN Categories ON Products.CategoryID = Categories.CategoryID;
</div>
<a target="_blank" href="trysql.asp?filename=trysql_select_join_inner_prod2" class="w3-btn w3-margin-bottom">
 Try it Yourself &raquo;</a>
</div>
<p>The example above works without specifying table names, because none of the 
specified column names are present in both tables.
However, if you add the CategoryID column in the 
<code class="w3-codespan">SELECT</code> statement, an error occurs, if you do not specify the table name. 
This is because the CategoryID column is present in both tables.</p>
<hr>

<h2>JOIN or INNER JOIN</h2>
<p><code class="w3-codespan">JOIN</code> and <code class="w3-codespan">INNER JOIN</code> will return the same result.</p>
<p><code class="w3-codespan">INNER</code> is the default join type for <code class="w3-codespan">JOIN</code>,
so when you write <code class="w3-codespan">JOIN</code> the parser actually writes <code class="w3-codespan">INNER JOIN</code>.</p>
<div class="w3-example">
<h3>Example</h3>
<p>JOIN is the same as INNER JOIN:</p>
<div class="w3-code notranslate sqlHigh">
SELECT Products.ProductID, Products.ProductName, Categories.CategoryName<br>
FROM Products<br>
JOIN Categories ON Products.CategoryID = Categories.CategoryID;
</div>
<a target="_blank" href="trymysql.asp?filename=trysql_select_join_inner_prod3" class="w3-btn w3-margin-bottom">
 Try it Yourself &raquo;</a>
</div>
<hr>

<h2>JOIN Multiple Tables</h2>
<p>You can join more than two tables by adding multiple <code class="w3-codespan">INNER JOIN</code> clauses in your query.</p>
<p>The following SQL selects all orders with customer and shipper information:</p>
...
<p>Here is the <a href="trysql.asp?filename=trysql_shippers" target="_blank"><strong>Shippers</strong></a> table:</p>
<div class="w3-responsive">
<table class="ws-table-all notranslate">
<tr>
<th>ShipperID</th>
...
<hr>
<div id="exercisecontainer" src="xrcise_join_inner.js"></div>
<hr>

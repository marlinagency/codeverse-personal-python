...
<html lang="en-US">
<head>
<title>Python Machine Learning - Grid Search</title>
<meta charset="utf-8">
...
<h1>Machine Learning - Grid Search</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_logistic_regression.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_preprocessing.asp">Next &#10095;</a>
</div>
<hr>
<h2>Grid Search</h2>

<p>The majority of machine learning models contain parameters that can be adjusted to vary how the model learns.
For example, the logistic regression model, from <code class="w3-codespan">sklearn</code>,
has a parameter <code class="w3-codespan">C</code> that controls regularization,which affects the complexity of the model.</p>

<p>How do we pick the best value for <code class="w3-codespan">C</code>?
...
<p>One method is to try out different values and then pick the value that gives the best score. This technique is known as a <strong>grid search</strong>.
...
<p>Before we get into the example it is good to know what the parameter we are changing does.
Higher values of <code class="w3-codespan">C</code> tell the model, the training data resembles real world information,
place a greater weight on the training data. While lower values of <code class="w3-codespan">C</code> do the opposite.</p>

<hr>

<h2>Using Default Parameters</h2>

<p>First let's see what kind of results we can generate without a grid search using only the base parameters.</p>

<p>To get started we must first load in the dataset we will be working with.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">from sklearn import datasets<br>
iris = datasets.load_iris()</code></p>
</div>

<p>Next in order to create the model we must have a set of independent variables X and a dependant variable y.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">X = iris['data']<br>
y = iris['target']</code></p>
</div>

<p>Now we will load the logistic model for classifying the iris flowers.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">from sklearn.linear_model import LogisticRegression</code></p>
</div>

<p>Creating the model, setting max_iter to a higher value to ensure that the model finds a result.</p>

<p>Keep in mind the default value for <code class="w3-codespan">C</code> in a logistic regression model is <code class="w3-codespan">1</code>, we will compare this later.</p>

<p>In the example below, we look at the iris data set and try to train a model with varying values for <code class="w3-codespan">C</code> in logistic regression.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">logit = LogisticRegression(max_iter = 10000)</code></p>
</div>

<p>After we create the model, we must fit the model to the data.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">print(logit.fit(X,y))</code></p>
</div>

<p>To evaluate the model we run the score method.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">print(logit.score(X,y))</code></p>
</div>

<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  from sklearn import datasets<br>from sklearn.linear_model import 
  LogisticRegression<br><br>iris = datasets.load_iris()<br><br>X = iris['data']<br>
  y = iris['target']<br><br>logit = LogisticRegression(max_iter = 10000)<br><br>
  print(logit.fit(X,y))<br><br>print(logit.score(X,y))</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_grid_search">Run example &raquo;</a>
</div>

<p>With the default setting of <code class="w3-codespan">C = 1</code>, we achieved a score of <code class="w3-codespan">0.973</code>.</p>

<p>Let's see if we can do any better by implementing a grid search with difference values of 0.973.</p>
...
<h2>Implementing Grid Search</h2>

<p>We will follow the same steps of before except this time we will set a range of values for <code class="w3-codespan">C</code>.</p>

<p>Knowing which values to set for the searched parameters will take a combination of domain knowledge and practice.</p>

<p>Since the default value for <code class="w3-codespan">C</code> is <code class="w3-codespan">1</code>, we will set a range of values surrounding it.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">C = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]</code></p>
</div>

<p>Next we will create a for loop to change out the values of <code class="w3-codespan">C</code> and evaluate the model with each change.</p>

<p>First we will create an empty list to store the score within.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">scores = []</code></p>
</div>

<p>To change the values of <code class="w3-codespan">C</code> we must loop over the range of values and update the parameter each time.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">for choice in C:<br>
&nbsp;
logit.set_params(C=choice)<br>
&nbsp;
logit.fit(X, y)<br>
&nbsp;
...
<p>With the scores stored in a list, we can evaluate what the best choice of <code class="w3-codespan">C</code> is.</p>

<div class="w3-example ws-light-grey">
  <p><code class="pythonHigh">print(scores)</code></p>
</div>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  ...
  LogisticRegression<br><br>iris = datasets.load_iris()<br><br>X = iris['data']<br>
  y = iris['target']<br><br>logit = LogisticRegression(max_iter = 10000)<br><br>
  C = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]<br><br>
  scores = []<br><br>for choice in C:<br>&nbsp; logit.set_params(C=choice)<br>&nbsp; 
  logit.fit(X, y)<br>&nbsp; scores.append(logit.score(X, y))<br><br>
  print(scores)</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_ml_grid_search2">Run example &raquo;</a>
</div>

<h2>Results Explained</h2>
<p>We can see that the lower values of <code class="w3-codespan">C</code> performed worse than the base parameter of <code class="w3-codespan">1</code>. However, as we increased the value of <code class="w3-codespan">C</code> to <code class="w3-codespan">1.75</code> the model experienced increased accuracy.</p>

<p>It seems that increasing <code class="w3-codespan">C</code> beyond this amount does not help increase model accuracy.</p>

<hr>

<h2>Note on Best Practices</h2>

<p>We scored our logistic regression model by using the same data that was used to train it. If the model corresponds too closely to that data, it may not be great at predicting unseen data. This statistical error is known as <strong>over fitting</strong>.</p>

<p>To avoid being misled by the scores on the training data, we can put aside a portion of our data and use it specifically for the purpose of testing the model. Refer to the lecture on train/test splitting to avoid being misled and overfitting.</p>
<hr>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_ml_logistic_regression.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_ml_preprocessing.asp">Next &#10095;</a>
